...
async def check_database_status():
    ...
    try:
        ...
        if table_exists:
            ...
            if page_stats and page_stats[0]['min_page']:
                stat = page_stats[0]
                print(f"\n  Page Range: {stat['min_page']} - {stat['max_page']} ({stat['unique_pages']} unique pages)")
            ...
            print("-" * 40)
            first_embedding_korean = await conn.fetchval(
                f"""
            ...
            first_embedding_english = await conn.fetchval(
                f"""
            ...
            if first_embedding_korean:
                # Parse the embedding string
                embedding_str = first_embedding_korean
            ...
            if first_embedding_english:
                # Parse the embedding string
                embedding_str = first_embedding_english
                if embedding_str.startswith('[') and embedding_str.endswith(']'):
                    embedding_values = embedding_str[1:-1].split(',')
